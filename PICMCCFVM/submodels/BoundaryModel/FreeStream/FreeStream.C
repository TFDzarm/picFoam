    ...
    particleFluxAccumulators_()
{
    ...
    forAll(molecules, i)
    {
        ...
        label typeId = findIndex(cloud.typeIdList(), molecules[i]);
        moleculeTypeIds_[i] = typeId;
        ...
        numberDensities_[i] /= cloud.constProps(typeId).nParticle();
    }
...
template<class CloudType>
void Foam::FreeStream<CloudType>::injection()
{
        ...
        forAll(patch, pFI)
        {
            ...
            forAll(pFA, i)
            {
                ...
                for (label i = 0; i < nI; i++)
                {
                    ...
                    typename CloudType::parcelType* parcel = cloud.addNewParcel(p, celli, U, typeId);
